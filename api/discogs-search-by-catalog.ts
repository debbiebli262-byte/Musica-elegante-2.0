...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    const rawCatno = String(req.query.catno || req.query.barcode || "").trim();
    ...
    const searchAttempts = (searchMode === "barcode"
      ? [
    ...
    const releaseArtists = Array.isArray(releaseData?.artists)
      ? releaseData.artists
          .map((a: DiscogsArtist) => String(a.name || "").replace(/\s*\(\d+\)$/, "").trim())
          .filter((name: string) => name && name !== "Various")
      : [];

    const tracklistArtists = Array.isArray(releaseData?.tracklist)
      ? releaseData.tracklist.flatMap((track: any) =>
    ...
    const artist = allArtistNames.length > 0 ? allArtistNames.join(", ") :
